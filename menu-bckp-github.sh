...
LLatest=`date`
Get_Data () {
git clone https://github.com/H-Pri3l/userbackup.git /root/user-backup/ &> /dev/null
}
...
Save_And_Exit () {
    cd /root/user-backup
    git config --global user.email "105465912+H-Pri3l@users.noreply.github.com" &> /dev/null
    ...
    git remote add origin https://github.com/H-Pri3l/userbackup
    git push -f https://ghp_BCugzEPypFU5MNGL17w41UcWxFw4F15sYSH@github.com/H-Pri3l/userbackup.git &> /dev/null
}
...
link="https://raw.githubusercontent.com/H-Pri3l/userbackup/main/$NameUser/$NameUser.zip"
sleep 1
...
cekdata=$(curl -sS https://raw.githubusercontent.com/H-Pri3l/userbackup/main/$NameUser/$NameUser.zip | grep 404 | awk '{print $1}' | cut -d: -f1)
...
mkdir -p /root/backup
wget -q -O /root/backup/backup.zip "https://raw.githubusercontent.com/H-Pri3l/userbackup/main/$NameUser/$NameUser.zip" &> /dev/null
